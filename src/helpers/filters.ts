...
export const detectMedia = (
  ...
  if (
    ...
    /<iframe[^>]+(?:youtube\.com|youtu\.be|bilibili\.com|vimeo\.com)/i.test(text)
  ) {
    media.add('video');
  }
...
export const buildFilters = (
  ...
  return [
      ...
      : []),
    {
      ...
      // 不列「长文」：本站的长文就是带标题的随记（撰写面板给随记加标题就存成
      // article），jant 的 FORMAT_ICONS 里同样只有 note/link/quote。
      // 归到「随记 › 有标题」下面，避免同一个东西出现两个入口。
      options: [
        { value: '', label: '全部形式', icon: 'shapes' },
        ...(kinds.some((kind) => kind === 'note' || kind === 'article')
          ? [
              { value: 'note', label: '随记', icon: KIND_ICONS.note },
              { value: 'note:titled', label: '有标题', icon: 'type', indent: true },
              { value: 'note:untitled', label: '无标题', icon: 'text', indent: true },
            ]
          : []),
        ...kinds
          .filter((kind) => kind !== 'note' && kind !== 'article')
          .map((kind) => ({
            value: kind,
...
export const ICON_PATHS: Record<string, string> = {
  ...
  sparkle: '<path d="M12 3 10.1 10.1 3 12l7.1 1.9L12 21l1.9-7.1L21 12l-7.1-1.9Z" />',
};
